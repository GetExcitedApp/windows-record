...
/// Gets the client area position and size (excluding borders and title bar)
pub fn get_window_rect(hwnd: HWND) -> Option<(i32, i32, u32, u32)> {
  unsafe {
    let mut client_rect = RECT::default();

    // Get the client rect (dimensions relative to the window)
    if GetClientRect(hwnd, &mut client_rect).as_bool() {
      // Use the client area dimensions
      ...
      let mut client_top_left = POINT { x: 0, y: 0 };
      if ClientToScreen(hwnd, &mut client_top_left).as_bool() {
        let left = client_top_left.x;
        let top = client_top_left.y;
        ...
        warn!(
          "Failed to convert client coordinates to screen coordinates for hwnd: {:?}, error: {:?}",
          hwnd, error_code
        );
        None
      }
      ...
      let error_code = windows::Win32::Foundation::GetLastError();
      warn!(
        "Failed to get client rect for hwnd: {:?}, error: {:?}",
        hwnd, error_code
      );
      None
    }
...
unsafe extern "system" fn window_enumeration_callback(hwnd: HWND, lparam: LPARAM) -> BOOL {
  ...
  let window_text = match context.match_type {
    WindowMatchType::Substring => String::from_utf16_lossy(&text[..length as usize]).to_lowercase(),

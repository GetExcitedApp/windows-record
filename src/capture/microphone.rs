...
unsafe fn setup_microphone_client_for_device(device_id: Option<&str>) -> Result<IAudioClient> {
  ...
  match coinit_result {
    ...
    Err(e) => {
      ...
      if e.code() != CO_E_ALREADYINITIALIZED
        && e.code() != windows::Win32::Foundation::RPC_E_CHANGED_MODE
      {
        return Err(e);
      }
      info!(
        "COM already initialized (possibly with different threading model by Electron): {:?}",
        e
      );
    }
  }
        ...
        match enumerator.GetDefaultAudioEndpoint(eCapture, eConsole) {
          ...
          Err(e) => {
            ...
            if error_message.contains("Element not found") {
              info!(
                "No microphone found on system. Error code: {:X} (Element not found)",
                e.code().0
              );
            } else {
  ...
  if let Err(e) = audio_client.GetDevicePeriod(Some(&mut default_period), Some(&mut minimum_period))
  {
...
unsafe fn create_microphone_sample(
  ...
  if wave_format.wBitsPerSample == 32 {
    let src = std::slice::from_raw_parts(buffer as *const f32, num_frames as usize * num_channels);
    let dst =
      std::slice::from_raw_parts_mut(buffer_data as *mut f32, num_frames as usize * num_channels);

    // Copy the samples
    std::ptr::copy_nonoverlapping(src.as_ptr(), dst.as_mut_ptr(), src.len());
  } else if wave_format.wBitsPerSample == 16 {
    let src = std::slice::from_raw_parts(buffer as *const i16, num_frames as usize * num_channels);
    let dst =
      std::slice::from_raw_parts_mut(buffer_data as *mut i16, num_frames as usize * num_channels);
